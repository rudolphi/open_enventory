...
var buttonName=new Array("bold","italic","underline","strikethrough","separator","subscript","superscript","makechem","separator","justifyleft","justifycenter","justifyright","justifyfull","separator","insertunorderedlist","insertorderedlist","separator","undo","redo");
// "cut","copy","paste","separator","indent","outdent","link","insertimage","separator",

// Order of available commands in toolbar2
// Remove from this any buttons not required in your application
var buttonName2=new Array("specialchar","makegreek","separator","forecolor","backcolor","separator","inserthorizontalrule","inserttable","separator","font","headers","removeformat","separator","upsize","downsize","separator","help");
// ,"separator","htmlmode","separator","removeformat"
...
function h2x(node,inPre) { // we will pass the node containing the Wyzz-generated html
	...
	for(i=0;i<node.childNodes.length;i++) {
		if (node.childNodes[i].parentNode && String(node.tagName).toLowerCase() !=String(node.childNodes[i].parentNode.tagName).toLowerCase()) {
			continue;
		}
		...
		case 1: // for element nodes
			...
			for(j=0;j<atts.length;j++) { // for each attribute
				...
				switch(attname) {
				case "style":
					attvalue=node.childNodes[i].style.cssText;
				break;
				case "class":
					attvalue=node.childNodes[i].className;
				break;
				case "name":
					attvalue=node.childNodes[i].name;
				break;
				default:
					try {
...
function wyzzHandleKB(e,n) { // n/a for IE
	...
	break;
	case 85: // u
		formatText("underline",n);
	break;
	case 109: // -
	case 222: // #
		formatText("subscript",n);
	break;
	case 56: // +
		formatText("superscript",n);
	break;
	case 76: // l
		formatText("justifyleft",n);
	break;
	case 67: // c
		formatText("justifycenter",n);
	break;
	}
}
...
function replaceGreek(n) {
	insertHTML(makeGreek(grabSelectedText(n)),n);
}

function replaceChem(n) {
	insertHTML(getBeautySum(grabSelectedText(n)),n);
}

function insertTable(n,rows,cols) {
	...
	var iHTML="<table border=\"1\" cellspacing=\"0\"><tbody>";
	for (var b=0;b<rows;b++) {
		iHTML+="<tr>";
		for (var c=0;c<cols;c++) {
			iHTML+="<td style=\"height:10pt;width:50pt;empty-cells:show\"> </td>";
		}
		iHTML+="</tr>";
	}
	iHTML+="</tbody></table>";
	hideObj('tablepicker'+n);
	insertHTML(iHTML,n);
}
...
function updateIframe(n) {
	// Pass the textarea's existing text into the editor
	var content=nl2br(document.getElementById(n).value,true),frm=document.getElementById("wysiwyg"+n);
...
function installListeners(n) {
	...
	doc.attachEvent('onkeyup', function() { valChanged(); });
        ...
        doc.body.contentEditable=true;
        doc.documentElement.addEventListener('keyup',function(event) { valChanged(); wyzzHandleKB(event,n); }, true);
	doc.documentElement.addEventListener('click',function(event) { doc.body.contentEditable=true; }, true);
    }
}
...
function make_wyzz(textareaID) {
	...
	for (var b=0;b<=max;b++) {
		...
		for (var c=0;c<=max;c++) {
			if (b==0) {
				...
				toolbar2+='<td onClick="insertTable(\''+n+'\','+b+','+c+');" style="cursor:pointer">&nbsp;</td>';
			}
		}
		toolbar2+='</tr>';
	}
	...
	//~ onFocus="fC(event,'+ref_obj+'); " onBlur="alert(0); updateTextArea(&quot;'+n+'&quot;); valChanged('+ref_obj+'); "
	...
	if (!isMSIE) {
		iframe+=' onload="installListeners(&quot;'+n+'&quot;);"';
	}
	iframe+='></iframe>\n'
	+'</td></tr></table>\n';
...
function formatTextColor(color,n,selected) {
	...
	performExec(n,'forecolor',false,color);
	hideObj('colorpicker'+n);
}
...
function formatBackColor(color,n,selected) {
	if (isMSIE) {
		...
		performExec(n,'backcolor',false,color);
	}
	else {
		//~ document.getElementById('wysiwyg'+n).contentWindow.document.execCommand('hilitecolor', false, color);
		performExec(n,'hilitecolor',false,color);
	}
	hideObj('colorbackpicker'+n);
}
...
function formatFontName(fontname,n,selected) {
	...
	performExec(n,'fontName',false,fontname);
	hideObj('fontpicker'+n);
}

function formatSpecialChar(charname,n,selected) {
	insertHTML(charname,n);
	hideObj('specialpicker'+n);
}
...
function formatHeader(headername,n,selected) {
	...
	performExec(n,'formatBlock',false,'<'+headername+'>');
	hideObj('headerpicker'+n);
}

function performExec(n,cmd,show_ui,value) {
	var obj=document.getElementById('wysiwyg'+n);
	if (obj) {
		obj.contentWindow.document.execCommand(cmd, show_ui,value);
	}
}
...
function formatText(id,n,selected) {
	if (mode==0&&id!='htmlmode') {
		...
		if (id=="inserttable") {
			//~ insertTable(n);
			toggleVisible('tablepicker'+n);
		}
		else if (id=="makegreek") {
			replaceGreek(n);
		}
		else if (id=="makechem") {
			replaceChem(n);
			...
			if (currentFontSize=='' || !currentFontSize) {
				currentFontSize=3; // fudge for FF
			}
			...
			performExec(n,'FontSize',false,newFontSize);
		}
		else if (id=="downsize") {
			var currentFontSize=document.getElementById("wysiwyg"+n).contentWindow.document.queryCommandValue("FontSize");
...
function updateTextArea(n) {
	if (xhtml_out==1) {
		document.getElementById(n).value=fixNL(h2x(document.getElementById("wysiwyg"+n).contentWindow.document.body));
	}
	else {
